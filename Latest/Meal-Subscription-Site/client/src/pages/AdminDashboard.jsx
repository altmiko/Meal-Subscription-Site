...
export default function AdminDashboard() {
	...
	const [reports, setReports] = useState({ topMeals: [], ordersPerDay: [], revenue: [] });
	...
	const roleOptions = useMemo(() => ['customer', 'restaurant', 'deliveryStaff'], []);
	...
	const updateDeliveryStatus = async (id, status) => {
		...
		setError('');
		try {
			await axiosInstance.patch(`/api/admin/deliveries/${id}`, { status });
	...
	const saveMealComment = async (id) => {
		...
		setError('');
		try {
			await axiosInstance.patch(`/api/admin/meals/${id}`, { adminComment: mealComments[id] ?? '' });
			...
			<div className="mb-6 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
				<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
					<div>
						...
						<p className="text-gray-600">Manage users, orders, deliveries, and reports</p>
					</div>
			...
			{activeTab === 'Overview' && (
				<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
					{[
						...
						{ label: 'Subscriptions', value: overview?.subscriptionCount },
								...
								{users.map((u) => (
									<tr key={u._id} className="border-b border-gray-100">
										...
										<td className="px-4 py-3">
											<button
												disabled={busyId === u._id}
												onClick={() => updateUserField(u._id, { isActive: !u.isActive })}
							...
							<tbody>
								{orders.map((o) => (
									<tr key={o._id} className="border-b border-gray-100">
										<td className="px-4 py-3 font-semibold text-gray-900">{o._id}</td>
										<td className="px-4 py-3 text-gray-700">{o.userId?.name || '—'}</td>
										<td className="px-4 py-3 text-gray-700">{o.restaurantId?.name || '—'}</td>
										<td className="px-4 py-3 text-gray-700">{o.total}</td>
										<td className="px-4 py-3">
											<select
							...
							<tbody>
								{deliveries.map((d) => (
									<tr key={d._id} className="border-b border-gray-100">
										<td className="px-4 py-3 font-semibold text-gray-900">{d._id}</td>
										<td className="px-4 py-3 text-gray-700">{d.order?._id || '—'}</td>
										<td className="px-4 py-3 text-gray-700">{d.customer?.name || '—'}</td>
										<td className="px-4 py-3 text-gray-700">{d.deliveryStaff?.name || '—'}</td>
										<td className="px-4 py-3">
											<select
												disabled={busyId === d._id}
												value={d.status}
												onChange={(e) => updateDeliveryStatus(d._id, e.target.value)}
												className="rounded-lg border border-gray-200 bg-white px-3 py-2"
											>
												{[
													'unassigned',
							...
							<tbody>
								{subscriptions.map((s) => (
									<tr key={s._id} className="border-b border-gray-100">
										<td className="px-4 py-3 font-semibold text-gray-900">{s._id}</td>
										<td className="px-4 py-3 text-gray-700">{s.user?.name || '—'}</td>
										<td className="px-4 py-3 text-gray-700">{s.planType}</td>
										<td className="px-4 py-3">
											<select
												disabled={busyId === s._id}
												value={s.status}
												onChange={(e) => updateSubscriptionStatus(s._id, e.target.value)}
												className="rounded-lg border border-gray-200 bg-white px-3 py-2"
											>
												{['active', 'paused', 'cancelled', 'expired'].map((st) => (
							...
							<tbody>
								{meals.map((m) => (
									<tr key={m._id} className="border-b border-gray-100">
										<td className="px-4 py-3 font-semibold text-gray-900">{m._id}</td>
										<td className="px-4 py-3 text-gray-700">{m.restaurant?.name || '—'}</td>
										<td className="px-4 py-3 text-gray-700">{m.name}</td>
										<td className="px-4 py-3 text-gray-700">{m.day}</td>
										<td className="px-4 py-3 text-gray-700">{m.mealType}</td>
										<td className="px-4 py-3 text-gray-700">{m.price}</td>
										<td className="px-4 py-3">
											<textarea
												disabled={busyId === m._id}
												value={mealComments[m._id] ?? ''}
												onChange={(e) =>
													setMealComments((prev) => ({ ...prev, [m._id]: e.target.value }))
												}
												rows={2}
											...
											{m.adminComment ? (
												<div className="mt-2 w-72 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 text-[11px] text-gray-700">
													<div className="font-semibold text-gray-900">Current comment</div>
													<div className="mt-1 whitespace-pre-wrap">{m.adminComment}</div>
												</div>
											) : null}
										</td>
			...
			{activeTab === 'Reports' && (
				<div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
					<div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
						<div className="text-sm font-semibold text-gray-900">Top meals</div>
					...
					<div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
						<div className="text-sm font-semibold text-gray-900">Orders per day (30d)</div>
						<div className="mt-3 space-y-2 text-sm text-gray-700">
					...
					<div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
						<div className="text-sm font-semibold text-gray-900">Revenue per day (30d)</div>
						<div className="mt-3 space-y-2 text-sm text-gray-700">
							...
							<tbody>
								{referrals.map((r) => (
									<tr key={r._id} className="border-b border-gray-100">
										<td className="px-4 py-3 text-gray-700">{r.referrer?.name || '—'}</td>
										<td className="px-4 py-3 text-gray-700">
											{r.referredUser?.name || '—'}
										</td>
										<td className="px-4 py-3 text-gray-700">{r.codeUsed}</td>
