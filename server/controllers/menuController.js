...
export const createMenuItem = async (req, res) => {
    try {
        ...
        const { name, description = '', price, calories, ingredients } = req.body;
        ...
        const item = await MenuItem.create({
            ...
            ingredients: Array.isArray(ingredients) ? ingredients : (typeof ingredients === 'string' && ingredients.length ? ingredients.split(',').map(i => i.trim()) : []),
            restaurant: userId,
            imageUrl,
        });
...
export const updateMenuItem = async (req, res) => {
    try {
        ...
        const { name, description, price, calories, ingredients, refetchImage } = req.body;
        ...
        if (ingredients !== undefined) {
            item.ingredients = Array.isArray(ingredients) ? ingredients : (typeof ingredients === 'string' && ingredients.length ? ingredients.split(',').map(i => i.trim()) : []);
        }
