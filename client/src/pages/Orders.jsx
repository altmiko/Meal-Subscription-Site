                ...
                <div key={order._id} className="group bg-white border border-stone-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col h-full">
                  ...
                  <div className="space-y-3 mb-6 flex-grow">
                    ...
                    {order.isSubscription && (
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${order.paymentStatus === 'paid' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                          {order.paymentStatus === 'paid' ? '✅' : '⏳'}
                        </div>
                        <div>
                          <div className="text-[10px] text-stone-400 font-bold uppercase tracking-wider mb-0.5">Payment Status</div>
                          <div className={`text-xs font-bold ${order.paymentStatus === 'paid' ? 'text-emerald-700' : 'text-amber-700'}`}>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <button
                              ...
                              className="px-3 py-2 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition font-bold text-xs shadow-lg shadow-emerald-100 disabled:opacity-50 disabled:grayscale"
                            >
                              🍳 Start Cooking
                            </button>
                            <button
