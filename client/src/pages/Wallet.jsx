...
export default function Wallet() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-stone-50 via-emerald-50/30 to-stone-100 flex items-center justify-center pt-24">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            ...
            className="rounded-lg border border-stone-300 bg-white px-4 py-2 text-sm font-medium text-stone-700 transition hover:border-emerald-300 hover:bg-emerald-50"
          >
            ← Back to Dashboard
          </button>
        </div>
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl p-6 mb-6 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-4xl font-bold">{wallet.balance.toFixed(0)} <span className="text-xl font-normal text-emerald-100">BDT</span></p>
            </div>
          ...
          <form onSubmit={handleRecharge} className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <input
                ...
                className="w-full border border-stone-300 rounded-lg px-4 py-3 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 focus:outline-none transition"
                placeholder="Enter amount (BDT)"
              />
            </div>
            <button
              type="submit"
              className="bg-emerald-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-emerald-700 transition shadow-sm"
            >
              Add to Wallet
            </button>
          </form>
          ...
          <div className="flex gap-2 mt-4">
            {[100, 500, 1000, 2000].map((amount) => (
              <button
                ...
                className="px-4 py-2 rounded-lg border border-stone-200 bg-stone-50 text-stone-700 text-sm font-medium hover:border-emerald-300 hover:bg-emerald-50 transition"
              >
                +{amount}
              </button>
            ))}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isCredit ? 'bg-emerald-100' : 'bg-rose-100'}`}>
